...
def makeGraph(fullList):
    quantity = 10
    tl = fullList[0: quantity]
    rl = [each[1] for each in fullList[0: quantity]] 
    people = tuple([each[0] for each in tl]) #people
    segments = 4
    ...
    for eachReasonList in rl:
        localList = [reason[0] for reason in eachReasonList[0: quantity]]
        plist.append(localList)
    ...
    for j in range(len(patch_handles)):
        for i, patch in enumerate(patch_handles[j].get_children()):
            ...
            x = 0.5*patch.get_width() + bl[0]
            y = 0.5*patch.get_height() + bl[1]
#             print('..indices ..', plist[i][j])
            ax.text(x,y, "%s" % (plist[i][j]), ha='center')
